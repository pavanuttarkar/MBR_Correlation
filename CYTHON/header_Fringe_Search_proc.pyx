...
cpdef extract_time(fil):
    ...
    '''
       Module to extract time tag from the file name, this is less reliable, as the file names
...
cpdef extract_series(fil):
    '''
       Module to extract series tag from the file name
...
cpdef gps_sync(file_name, file_name1, fact):
    '''
       ...
       Module to calculate synchronization solution for the given 000 series data set
       if extracts using the synchronization series Info files generated using the
       'super_sync_generator' module. This can oly be used for 000 series file and
       ...
       Further Reference:
           Refer mocule super_sync_generator
...
cpdef extract_XY_vals(file_name, file_name1):
    '''
       Module to extract polarization and DSP numbers from the file name
