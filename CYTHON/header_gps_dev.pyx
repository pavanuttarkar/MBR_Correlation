...
cpdef super_sync_generator(*args):
    '''
...
cpdef fil_list(fil1):
    '''
        This module is a companinon of the high level module
        'super_sync_file', used to calculate the slopes of the
        files staraight line curve (GPS blip vs packet).     
        
       Parameters
       ----------
       fil1  : filename, with path if not local.
           path containing list of files
       Returns
       -------
       comf_X: 1-D numpy array
           array of X-Polarization voltage values.
