...
cpdef read_RFI(file_name, file_name1):
    ...
    RFI_mask1 = np.loadtxt(
        str("SAMPLING_INFO/RFI_mask_X_" + str(fil_000.split("/")[-1]))
    )  # +fillist[0])
    RFI_mask2 = np.loadtxt(
        str("SAMPLING_INFO/RFI_mask_Y_" + str(fil_000.split("/")[-1]))
    )
    RFI_mask3 = np.loadtxt(
        str("SAMPLING_INFO/RFI_mask_X_" + str(fil1_000.split("/")[-1]))
    )  # +fillist[0])
    RFI_mask4 = np.loadtxt(
        str("SAMPLING_INFO/RFI_mask_Y_" + str(fil1_000.split("/")[-1]))
    )
