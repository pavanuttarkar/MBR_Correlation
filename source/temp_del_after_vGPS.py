...
def Correlation_FORT(comf_X_1, comf_Y_1, comf1_X_1, comf1_Y_1, avg, le, file_name, file_name1, fftsize, external_num, c9, fil, delay, RFI):
    ...
    timin      =       time.time()
    creal1, creal2, creal3, creal4, creal5, creal6, creal8, creal9, pollkg1, pollkg2 = internal_loop_RFI.external_loop_rfi(comf_X_1, comf_Y_1, comf1_X_1, comf1_Y_1, avg, le, RFI[0], RFI[2], RFI[1], RFI[3])#, delay, 60000)
...
def main_controller():
    """
    Function responsible to control all fortran and cython instances.
    """
    if(len(sys.argv) < 7):
